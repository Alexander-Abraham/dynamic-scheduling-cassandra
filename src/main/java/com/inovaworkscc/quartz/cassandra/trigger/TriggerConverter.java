...
public class TriggerConverter {
     ...
     * @param row cassandra {@link Row} to read from.
     ...
     */
    public OperableTrigger toTrigger(TriggerKey triggerKey, Row row)
            throws JobPersistenceException {
        OperableTrigger trigger = toTriggerWithOptionalJob(triggerKey, row);
    ...
    private void loadCommonProperties(TriggerKey triggerKey, Row triggerRow, OperableTrigger trigger) {
        ...
        trigger.setNextFireTime(getInstant(triggerRow.getInstant(Constants.TRIGGER_NEXT_FIRE_TIME)));
       
        trigger.setPreviousFireTime(getInstant(triggerRow.getInstant(TRIGGER_PREVIOUS_FIRE_TIME)));
        trigger.setPriority(triggerRow.getInt(TRIGGER_PRIORITY));
    }

    private Date getInstant(Instant instant) {
    	if(instant==null) {
    		return null;
    	}
    ...
    private void loadStartAndEndTime(Row triggerRow, OperableTrigger trigger) {
        try {
            ...
            trigger.setStartTime(getInstant(triggerRow.getInstant(TRIGGER_START_TIME)));
            trigger.setEndTime(getInstant(triggerRow.getInstant(TRIGGER_END_TIME)));

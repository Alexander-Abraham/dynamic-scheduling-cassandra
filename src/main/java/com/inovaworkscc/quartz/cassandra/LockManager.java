    ...
    public boolean relockExpired(TriggerKey key) {
        ...
        if (existingLock != null) {
            if (expiryCalculator.isTriggerLockExpired(existingLock)) {
                ...
                return locksDao.relock(key,  Date.from(existingLock.getLocalDate(Constants.LOCK_TIME).atStartOfDay(ZoneId.systemDefault()).toInstant()));
            } else {
                log.info("Trigger {} hasn't expired yet. Lock time: {}",
                        key,  Date.from(existingLock.getLocalDate(Constants.LOCK_TIME).atStartOfDay(ZoneId.systemDefault()).toInstant()));
            }
        } else {
